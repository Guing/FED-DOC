...
const { readFile, writeFile, spinnerStart } = require('@black-cli/utils')
const request = require('@black-cli/request')
const CloudBuild = require('@black-cli/cloudbuild')
    ...
    async checkGitOwner() {
        ...
        if (!owner || !login || this.refreshOwner) {
            owner = (await inquirer.prompt({
                ...
                choices: this.orgs.length > 0 ? GIT_OWNER_TYPE : GIT_OWNER_TYPE_ONLY //如果获取到的组织列表为空，则不必要选择组织了。
            })).owner
...
`)
    ...
    async initCommit() {
        ...
        if (await this.checkRemoteMaster()) { //判断远程仓库master分支是否已存在
            await this.pullRemoteRepo('master', {
                '--allow-unrelated-histories': null  //这个参数：将本地仓库的master与远程仓库的master分支合并关联
            })
        } else {
