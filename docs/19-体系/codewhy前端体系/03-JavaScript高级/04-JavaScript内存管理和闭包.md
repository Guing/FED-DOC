...
- **分代收集（Generational collection）**—— 对象被分成两组：“新的”和“旧的”。
  - 一般创建新对象，新对象会马上使用，使用完就会清理掉。
  - 如果对象长期存活，则证明对象可能会一直存在，则对象会变得“老旧”，迁移到旧的组，而且被检查的频次也会减少；
...
- **在计算机科学中对闭包的定义（维基百科）：**
  ...
  - 闭包在实现上是一个结构体，它存储了**一个函数和一个关联的环境**（相当于一个符号查找表）；
...
![截屏2023-07-06 18.09.46](image/04-JavaScript%E5%86%85%E5%AD%98%E7%AE%A1%E7%90%86%E5%92%8C%E9%97%AD%E5%8C%85/%E6%88%AA%E5%B1%8F2023-07-06%2018.09.46.png)
...
- V8对于没有引用的AO属性会删除掉，但是safari浏览器不会，继续保留
