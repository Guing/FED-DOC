...
- 我们先测试一个知识点：如果一个组件中**含有多个插槽，我们插入多个内容时是什么效果？**
  - 我们会发现**默认情况下每个插槽都会获取到我们插入的内容来显示**；
...
- 在开发中，我们构建了组件树之后，除了父子组件之间的通信之外，还会有**非父子组件之间**的通信。
- **这里我们主要讲两种方式：**
  - 全局事件总线；
  - Provide/Inject；
...
- **Vue3从实例中移除了 $on、$off 和 $once 方法，所以我们如果希望继续使用全局事件总线，要通过第三方的库**：
  - Vue3官方有推荐一些库，例如[mitt](https://github.com/developit/mitt)或[tiny-emitter；](https://github.com/scottcorgan/tiny-emitter)
...
![](./image/Aspose.Words.7744bdd7-32c7-4dad-acc2-6117f15d4e08.032.png)
...
### 如何进行非父子组件的通信？（整理除了Vuex/Pinia其他方式）
