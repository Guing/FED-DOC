...
1.首先vue的响应式原理，分为三个：定义响应式数据，依赖收集，触发更新。
2.定义响应式数据时，我们先确定数据结构是怎么样的，在vue中，是先weakMap,把传入的对象作为key，再新建map作为值，map里就保存着对象的每个属性，作为key，再新建一个依赖收集对象，命名为depend。这个depend对象，类似于发布订阅机制，有添加函数，触发函数。
3.接着是依赖收集，在vue中，template模板会转化为render函数。我们可能通过建立一个watch函数，来执行这个render函数，当render函数执行时，会触发响应式数据里的get函数，在get函数里里，通过我们之前的数据结构，就可以obj,key，获取这个key的depend对象。depend对象通过add方法，把这个render函数添加到当前的依赖更新数组里面。
4.接下来是触发更新，当我们修改对象时，就会触发set方法，set方法里面，通过对象，key找到depend，触发更新。
