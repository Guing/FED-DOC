...
const MYSQL_CONNECTION_STR = "root:1417789619@tcp(server.lan.blackfe.com:3306)/minigame?charset=utf8mb4&parseTime=false"
...
func ConnectMySQL() *xorm.Engine {
	if engine == nil {
		...
		if strings.Compare(os.Getenv("DEBUG"),"true") == 1 {
			// 如果有新的模型需要同步，加在后面
			err := engine.Sync2(new(History), new(Account))
			if err != nil {
				panic(err)
			}
		}
